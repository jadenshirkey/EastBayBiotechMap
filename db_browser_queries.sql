 ...
 SELECT
      ...
      COALESCE(c.city, '(No City Listed)') as city,
      c.company_name,
      ...
      -- Classification
      COALESCE(cc.company_stage, 'Unknown') as company_stage,

      -- Clinical Trials Summary
      COALESCE(trial_stats.total_trials, 0) as total_trials,
      COALESCE(trial_stats.active_trials, 0) as active_trials,
      trial_stats.trial_phases,

      -- SEC Information
      sec.ticker as stock_ticker,
      sec.exchange,
      COALESCE(sec.filing_count, 0) as sec_filings,
      sec.latest_filing_date,
      ...
      -- Data Sources
      CASE
          WHEN bpg.company_id IS NOT NULL THEN 'BioPharmGuy'
          ELSE ''
      END ||
      CASE
          WHEN wiki.company_id IS NOT NULL THEN
              CASE WHEN bpg.company_id IS NOT NULL THEN ', ' ELSE '' END || 'Wikipedia'
          ELSE ''
      END as data_sources,
  ...
  LEFT JOIN company_classifications cc
      ON c.company_id = cc.company_id AND cc.is_current = 1
  ...
  LEFT JOIN (
      SELECT
          ...
          COUNT(CASE WHEN trial_status LIKE '%ACTIVE%' THEN 1 END) as active_trials,
          GROUP_CONCAT(DISTINCT phase) as trial_phases
      FROM clinical_trials
      GROUP BY company_id
  ) trial_stats ON c.company_id = trial_stats.company_id

  -- SEC Edgar data
  LEFT JOIN sec_edgar_data sec ON c.company_id = sec.company_id
  ...
  LEFT JOIN (
      ...
      GROUP BY company_id
  ) focus ON c.company_id = focus.company_id

  -- Source mapping to identify data origin
  LEFT JOIN (
      SELECT DISTINCT company_id
      FROM company_source_mapping
      WHERE source_type = 'biopharmguy'
  ) bpg ON c.company_id = bpg.company_id

  LEFT JOIN (
      SELECT DISTINCT company_id
      FROM company_source_mapping
      WHERE source_type = 'wikipedia'
  ) wiki ON c.company_id = wiki.company_id

  -- Order by city first, then alphabetically by company name within each city
  ORDER BY
      CASE
          WHEN c.city IS NULL OR c.city = '' THEN 'ZZZZ'  -- Put empty cities at the end
          ELSE c.city
      END,
      c.company_name;
